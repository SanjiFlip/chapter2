...
import numpy as np
import math
import SSA.test_fun_new as test_fun_new
import matplotlib.pyplot as plt
import SSA.test_function as test_function
    ...
    def update_position_three(self, c1):
        for i in range(0, self.pop):
            ...
            if (i <= self.pop / 2):  # 领导者比例
                ...
                for j in range(0, self.n_dim):
                    self.X[i, j] = np.clip(((self.X[i - 1, j] + w * self.X[i, j]) / 2), self.lb[j], self.ub[j])
        self.Y = [self.func(self.X[i]) for i in range(len(self.X))]  # y = f(x) for all particles
...
if __name__ == '__main__':
    ...
    """
        针对于 多峰函数优化较好
        test_fun_new F4 F8 F9 F10 F11(一般) F12
    """
    ...
    # print('best_y is', ssa1.gbest_y)
    # print('shuaitui best_y is', ssa2.gbest_y)
    # print('2 best_y is', ssa5.gbest_y)
    # print('chafen best_y is', ssa6.gbest_y)

    # print(f'{demo_func(ssa1.gbest_x)}\t{ssa1.gbest_x}')
    plt.plot(ssa1.gbest_y_hist, linestyle='-', color='b', label="original_SSA")
    plt.plot(ssa2.gbest_y_hist, linestyle='-', color='r', label="F4_Attention factor")
    plt.plot(ssa3.gbest_y_hist, linestyle='-', color='y', label="F4_Random weights")
    # plt.plot(ssa4.gbest_y_hist, linestyle='-', color='y', label="new_SSA3")
    plt.plot(ssa5.gbest_y_hist, linestyle='-', color='m', label="F4_Attention factor+Differential")
    plt.plot(ssa6.gbest_y_hist, linestyle='-', color='g', label="F10_Differential")
