...
class salp_swarm_algorithm():
    def __init__(self, pop_size=50, n_dim=2, max_iter=150, lb=None, ub=None, func=None):
        # 樽海鞘种群数量
        self.pop = pop_size
        # 为求解下边界
        self.lb = lb
    ...
    def update_position_three(self, c1, t):
        for i in range(0, self.pop):
            # 随机权重
    ...
    def run3(self):
        for i in range(self.max_iter):
            ...
            """
            c1 = 2 * math.exp(-(4 * ((i + 1) / self.max_iter)) ** 2)
            self.update_position_three(c1, i)
            self.update_pbest()
            self.update_gbest()
            self.gbest_y_hist.append(self.gbest_y)
        self.best_x, self.best_y = self.gbest_x, self.gbest_y
...
if __name__ == '__main__':
    ...
    """
        ...
        F13 -50,50
    """
    lb = [-500 for i in range(n_dim)]
    ub = [500 for i in range(n_dim)]
    """
        针对于 多峰函数优化较好
        test_fun_new F4(非多峰) F8 F9 F10 F11 F12 F13
    """
    ...
    # plt.plot(ssa2.gbest_y_hist, linestyle='-', color='r', label="Decay_factor")
    plt.plot(ssa3.gbest_y_hist, linestyle='-', color='g', label="Decay_factor+RandomWeight")
    # plt.plot(ssa4.gbest_y_hist, linestyle='-', color='y', label="new_SSA3")
    plt.plot(ssa5.gbest_y_hist, linestyle='-', color='m', label="Decay_factor+Differential")
    # plt.plot(ssa6.gbest_y_hist, linestyle='-', color='g', label="Differential")
